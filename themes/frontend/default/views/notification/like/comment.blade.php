...
<div class="user media user-mini notification {{$notification->id}}-notification">
    ...
    <div class="media-body">
        <h5 class="media-heading">
            ...
            <a href="{{$notification->user->present()->url()}}">{{$notification->user->fullname}} </a> {{trans('notification.like-your')}} <a data-ajaxify="true" href="{{URL::route('post-page', ['id' => $comment->post->id])}}">{{trans('comment.comment')}}</a>
        </h5>
