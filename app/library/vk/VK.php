...
class VK
{
    ...
    public function getAuthorizeUrl($api_settings = '',
        $redirect_uri = 'https://api.vk.com/blank.html', $test_mode = false)
    {
        $parameters = array(
     ...
     * @return  array
     */
    public function getAccessToken($code, $redirect_uri = 'https://api.vk.com/blank.html')
    {
        ...
        if (isset($rs['error'])) {
            throw new VKException($rs['error'].(!isset($rs['error_description']) ?'' : ': ' . $rs['error_description']));
        } else {
            $this->auth = true;
